...
pub struct EntryHeuristics {
    ...
    /// Whether this route is marked as a priority route.
    pub priority: bool,
}

impl EntryHeuristics {
    /// Heuristics for an entry that is a priority route: belongs to no clusters and is marked as a
    /// priority route.
...
pub async fn compute_chunk_group_info(graph: &ModuleGraph) -> Result<Vc<ChunkGroupInfo>> {
    ...
    async move {
        ...
        let mut inherits_from: FxHashMap<usize, RoaringBitmap> = FxHashMap::default();
                        ...
                        let chunk_group_ids = chunk_groups.map(|chunk_group| {
                                ...
                                && let Some(parent_groups) = module_chunk_groups.get(&parent_module)
                            {
                                for source in parent_groups.iter() {
                                    inherits_from.entry(source as usize).or_default().insert(id);
                                }
                            }
                            id
                        });
        ...
        for chunk_group in &entries {
            ...
            if heuristics.clusters.is_empty() && !heuristics.priority {
                continue;
            }
            if let Some(index) =
        ...
        while let Some(source) = worklist.pop() {
            ...
            for target in targets.iter() {
                ...
                *target_clusters |= &*source_clusters;
                let mut changed = previous_target_clusters_len != target_clusters.len();
                if source_priority_route && priority_routes.insert(target as u32) {
                    changed = true;
                }
                if changed {
